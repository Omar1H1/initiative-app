...
const Navbar = () => {
    ...
    const newNotificationsCount = notifications.filter(notification => !notification.seen).length;
    ...
    return (
        <div className="bg-white dark:bg-gray-800 fixed top-0 left-0 w-full z-50 flex justify-between items-center h-24 px-4 text-white shadow-md">
            ...
            <ul className="hidden md:flex items-center">
                {token && (
                    ...
                    >
                        <IoNotifications
                            size={24}
                            className="hover:bg-blue-400 hover:text-white rounded-full text-black dark:text-gray-200"
                        />
            ...
            {/* Mobile Menu Icon */}
            <div onClick={handleNavToggle} className="block md:hidden cursor-pointer text-black dark:text-gray-200">
                {nav ? <AiOutlineClose size={24} /> : <AiOutlineMenu size={24} />}
            </div>

            {/* Mobile Navigation */}
            <ul
                className={`fixed top-0 left-0 w-[60%] h-full border-r border-gray-700 bg-white dark:bg-gray-900 transform ${nav ? "translate-x-0" : "-translate-x-full"
            ...
            >
                ...
                {token && (
                    <li className="m-4 cursor-pointer" onClick={handleNotificationClick}>
                        <IoNotifications size={24} className="inline-block mr-2 text-black dark:text-gray-200" />
