...
const useNotifications = (userId: string | undefined) => {
    const [notifications, setNotifications] = useState<{ id: number; title: string; info: string }[]>([]);
    ...
    useEffect(() => {
        ...
        const eventSource = new EventSource(`http://localhost:8080/api/v1/notifications/match?userId=${userId}`);
